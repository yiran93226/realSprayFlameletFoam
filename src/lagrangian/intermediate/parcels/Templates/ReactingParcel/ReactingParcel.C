...
)
{
    ...
    //test
    // Info<< "Here we update the particle temperature: " << endl;
    ...
    forAll(dMassPC, i)
    {
        kappaEff += composition.liquids().properties()[i].kappa(td.pc(), this->T_);
        mu_L += composition.liquids().properties()[i].mu(td.pc(), this->T_);
        alphah_L += composition.liquids().properties()[i].alphah(td.pc(), this->T_);
    }
    ...
    scalar htc = cloud.heatTransfer().htc(this->d_, Res, Prs, kappas, NCpW); 

    // Ts = this->T_;
    // scalar Ql_1 = kappaEff / delta_thermal * this->areaS(this->d_) * (Ts - this->T_);
    // // scalar Ql_2 = htc * this->areaS(this->d_) * (td.Tc() - Ts) + Sh;
    // scalar Ql_2 = htc * this->areaS(this->d_) * (td.Tc() - Ts);
    // Info<< "Ql_1 = " << Ql_1 << ", Ql_2 = " << Ql_2 << endl;
    scalar Ql_1 = 0.0;
    scalar Ql_2 = 0.0;

    for (int j = 0; j < 50; j++)
    {
        // Ql_1 = mag(kappaEff / delta_thermal * this->areaS(this->d_) * (Ts - this->T_));
        // Ql_2 = mag(htc * this->areaS(this->d_) * (td.Tc() - Ts) + Sh);
        Ql_1 = kappaEff / delta_thermal * this->areaS(this->d_) * (Ts - this->T_);
        Ql_2 = htc * this->areaS(this->d_) * (td.Tc() - Ts) + Sh;
        // Info<< "Ql_1 = " << Ql_1 << ", Ql_2 = " << Ql_2 << endl;

        if ( mag( (Ql_1 - Ql_2)/max(small, Ql_1 + Ql_2) ) < 1e-4 )
        {
            // Info<< "iteration number j = " << j << ", Ts = " << Ts << endl;
            // Info<< "Ql_1 = " << Ql_1 << ", Ql_2 = " << Ql_2 << endl;
            break;
        }

        Ts += -(Ql_1 - Ql_2)*dt/(this->mass()*this->Cp_);
        Ts = min(max(Ts, cloud.constProps().TMin()), cloud.constProps().TMax());

        // if (j == 49)
        // {
        //     Info<< "iteration of Ql is not converged." << endl;
        //     Info<< "Ts = " << Ts << endl;
        //     Info<< "Ql_1 = " << Ql_1 << ", Ql_2 = " << Ql_2 << endl;
        // }        
    }    
    ...
    scalar deltaT_Ql = mag(Ql_1) * delta_thermal / (kappaEff * this->areaS(this->d_));
    this->T_ = min(max(Ts - deltaT_Ql, cloud.constProps().TMin()), cloud.constProps().TMax());

    //Ql should be large than 0
    
    // scalar Tnew = min(max(Ts, cloud.constProps().TMin()), cloud.constProps().TMax()) - mag(Ql) * delta_thermal / (dt * kappaEff * pi * sqr(this->d_));
    // this->T_ = min(max(Tnew, cloud.constProps().TMin()), cloud.constProps().TMax());

    //test    
    // Info<< "Ts = " << Ts << ", Tnew = " << Tnew << ", Ql = " << Ql << ", chi = " << chi << ", delta_thermal = " << delta_thermal << ", kappaEff = " << kappaEff << endl;
    // Info<< "Ql changes the temperature deltaT = " << deltaT_Ql << endl;
    Info<< "particle temperature = " << this->T_ << endl;
    // Info<< "dhsTrans/dt = " << dhsTrans/dt << endl;
